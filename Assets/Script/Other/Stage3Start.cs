...
public class Stage3Start : MonoBehaviour
{
    ...
    private IEnumerator stageStart()
    {
        ...
        while (t < 1)
        {
            ...
            player.localPosition = new Vector3(player.localPosition.x, player.localPosition.y, player.localPosition.z + t);
            yield return null;
        }
        transform.localPosition = new Vector3(transform.localPosition.x, transform.localPosition.y, 5.8f);
        yield return new WaitForSeconds(1);
